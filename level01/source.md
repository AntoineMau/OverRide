...
int verify_user_name() {
    return strncmp("dat_wil", user_name, 200);
}

int32_t verify_user_pass(char *user_pass) {
    return strcmp("admin", user_pass);
}
...
int main(void)
{
    ...
    fgets(user_name, 0x100, _stdin);
    if (verify_user_name() == 0) {
        puts("Enter Password: ");
        fgets(user_pass, 100, _stdin);
        if (verify_user_pass(user_pass) != 0)
